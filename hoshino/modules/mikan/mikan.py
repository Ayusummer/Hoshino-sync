    ...
    async def get_rss():
        ...
        try:
            resp = await aiorequests.get(
                "https://mikanani.me/RSS/MyBangumi",
        ...
        for i in rss.xpath("/rss/channel/item"):
            ...
            pubDate = i.find(
                ".//xmlns:pubDate", namespaces={"xmlns": "https://mikanani.me/0.1/"}
            ).text
            pubDate = pubDate[:19]
...
async def mikan_poller():
    ...
    if not new_bangumi:
        ...
        msg = [
            f'{i[1]} 【{i[2].strftime(r"%Y-%m-%d %H:%M")}】\n▲下载 {i[0]}'
            for i in new_bangumi
        ]
        randomiser = (
            lambda m: f'{random.choice(DEVICES)}监测到番剧更新!{"!"*random.randint(0,4)}\n{m}'
        )
...
async def send_bangumi(bot, ev):
    ...
    msg = [
        f'{i[1]} 【{i[2].strftime(r"%Y-%m-%d %H:%M")}】\n▲链接 {i[0]}'
        for i in Mikan.rss_cache[: min(5, len(Mikan.rss_cache))]
    ]
    msg = "\n".join(msg)

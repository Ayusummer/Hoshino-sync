...
sv = Service('fgoavatarguess', help_='''
猜从者 | 猜猜机器人随机发送的头像的一小部分来自哪位角色
'''.strip())
...
async def avatar_guess(bot, ev: CQEvent):
    try:
        ...
        chara_id_list = list(_data.CHARA_NAME.keys())
        while True:
    ...
    try:
        if winner_judger.get_on_off_status(ev.group_id):
            ...
            if cid != chara.UNKNOWN and cid == winner_judger.get_correct_chara_id(
                ...
                c = OtherChara(winner_judger.get_correct_chara_id(ev.group_id))
                msg = f'正确答案是: {c.name}{c.icon.cqcode}\n{msg_part}'
                await bot.send(ev, msg)
    except Exception as e:
        await bot.send(ev, '错误:\n' + str(e))
